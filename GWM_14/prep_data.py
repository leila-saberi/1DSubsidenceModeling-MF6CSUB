...
def prep_data(use_delay, **kwargs):
    ...
    obs = pd.read_csv(os.path.join(w_d, 'source_data', '{0}_obs_data.csv'.format(location)), index_col=0,
                      parse_dates=True)
    lith = pd.read_csv(os.path.join(w_d, 'source_data', '{0}_lithology.csv'.format(location))).dropna(axis=1,how='all').dropna(axis=0)
...
def modify_pst(tpl_dir):
    ...
    cobs1 = cobs.loc[cobs.apply(lambda x: x.datetime.year <= 2024 and x.layer == 1, axis=1), :]
    assert cobs1.shape[0] > 0
    cobs1.sort_values(by="datetime", inplace=True)
    obs.loc[cobs1.index, "obsval"] = 0.5
    obs.loc[cobs1.index, "obgnme"] = "less-than-compact1"
    ...
    if phi_file is not None:
        ...
        if "less_than_rebound" not in df.tag.values:
            df.index = df.pop("tag")
            df.loc["less-than-compact1", "prop"] = 1
