...
def plot_wls(**kwargs):
    ...
    # Create a stress period for whenever we have a head measurement from either Lower or Upper aquifer
    # If there is not a measurement at both levels, we will interpolate for the missing level
    # Alternatively, the time-series will be resampled to specified frequency, freq
    tsdf = pd.DataFrame(index=obs.index.unique())
    for aq in obs.Aquifer.unique():
        tsdf = (
            tsdf.merge(
                obs.loc[obs.Aquifer == aq],
                how="outer",
                left_index=True,
                right_index=True,
            )
        ...
        tsdf.rename(columns={"Alt": aq}, inplace=True)
    tsdf = tsdf[
        [
            aq
            for aq in obs.Aquifer.unique()
            if any(lith.Aquifer.str.contains(aq.lower()))
        ]
    ].drop_duplicates()

    tsdf = tsdf.loc[~tsdf.index.duplicated(keep="first")]
    uaq = obs.Aquifer.unique()
    uaq.sort()
    with PdfPages(os.path.join(w_d, "processed_data", "gwlevels_types.pdf")) as pdf:
        fig, axes = plt.subplots(len(uaq), 1, figsize=(10, 10))

        for aq, ax in zip(uaq, axes.reshape(-1)):
            if aq == "Composite":
                continue
            uobs = obs.loc[obs.Aquifer == aq, :].copy()
            uobs.sort_index(inplace=True)
            ax.scatter(uobs.index, uobs["Alt"], color="r", label="raw")
            print(aq, obs.columns, tsdf.columns)
            for key, fcn in kwargs.items():
                moddf = fcn(tsdf)

                ax.plot(moddf.index, moddf[aq], label=key)
