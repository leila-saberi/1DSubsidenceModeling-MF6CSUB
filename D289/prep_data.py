...
def prep_data(use_delay, **kwargs):
    ...
    nlay = 2  # upper and lower aquifer
...
def modify_pst(tpl_dir):
    ...
    obs.loc[cobs2.index, "obsval"] = 50
    obs.loc[cobs2.index, "standard_deviation"] = 2.5
    obs.loc[cobs2.index, "obgnme"] = "greater_than-compact2"
    obs.loc[cobs2.index, "weight"] = 0
    ...
    if phi_file is not None:
        ...
        if "less_than_rebound" not in df.tag.values:
            df.index = df.pop("tag")
            df.loc["greater_than-compact2", "prop"] = 1.0
