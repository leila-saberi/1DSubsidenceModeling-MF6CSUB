...
def prep_data(use_delay, **kwargs):
    ...
    sub_data = pd.read_csv(os.path.join(w_d, "source_data", "{0}_sub_data.csv".format(location)), index_col=0,
                           parse_dates=True)
    sub_data["should_assimilate"] = True
    # sub_data.loc[sub_data.index<pd.to_datetime("1-1-1970"),"should_assimilate"] = False
    sub_data.to_csv(os.path.join(w_d, "processed_data", "{0}_sub_data.csv".format(location)))

    par_data = pd.read_excel(os.path.join(w_d, 'source_data', '{0}_par_data.xlsx'.format(location)))
    par_data = par_data.loc[:, ["parameter", "Upper", "Middle", "Lower"]]
    par_data.index = par_data.pop("parameter").values

    obs = pd.read_csv(os.path.join(w_d, 'source_data', '{0}_obs_data.csv'.format(location)), index_col=0,
                      parse_dates=True)
    obs.loc[-1] = [np.nan, -10, "Lower"]
    ivals = obs.index.tolist()
    ivals[-1] = pd.to_datetime("9-24-2002")
    obs.index = ivals

    obs.loc[-1] = [np.nan, -10, "Lower"]
    ivals = obs.index.tolist()
    ivals[-1] = pd.to_datetime("9-24-2004")
    obs.index = ivals

    lith = pd.read_csv(os.path.join(w_d, 'source_data', '{0}_lithology.csv'.format(location))).dropna(axis=1,
                                                                                                      how='all').dropna(
        axis=0)
    ...
    uaq = ["Upper", "Middle", "Lower"]
    uk = [0, 1, 2]
    smooth_thres = pd.to_datetime("1-1-1995")
    top = None
    for k, aq in zip(uk, uaq):
        ...
        aobs_interp_smooth = aobs_interp.rolling(window=365, min_periods=1, center=True).mean()
        aobs_interp.loc[aobs_interp.index.map(lambda x: x < smooth_thres), :] = aobs_interp_smooth.loc[
                                                                                aobs_interp_smooth.index.map(
                                                                                    lambda x: x < smooth_thres), :]
    ...
    if use_delay:
        lay_df.loc['cdelay', :] = ['nodelay'] + ['delay' for _ in range(len(lay_df.columns) - 1)]
    else:
    ...
    for aq in lay_df.columns:
        ...
        clay_lith = lith.loc[(lith.Aquifer == aq) & (lith.Description.str.contains('clay'))]
        lay_df.loc['clay_thickness', aq] = clay_lith.Thick_ft.sum()
        bequiv = np.sqrt((clay_lith.Thick_ft.values ** 2).sum() / float(len(clay_lith)))
        rnb = lay_df.loc['clay_thickness', aq] / bequiv
        if use_delay:
            thick_frac = bequiv
        else:
            thick_frac = clay_lith.Thick_ft.values.sum()
        lay_df.loc['thick_frac', aq] = thick_frac
        lay_df.loc['rnb', aq] = rnb
        lay_df.loc['clay_lith', aq] = len(clay_lith)
        lay_df.loc["clay_thickness_squared", aq] = (clay_lith.Thick_ft.values ** 2).sum()

        last = b

    botarr = np.array(botm)
    ...
    lay_df.to_csv(os.path.join(w_d, 'processed_data', f'{location}.model_property_data.csv'))
...
def modify_pst(tpl_dir):
    ...
    cobs3 = cobs.loc[cobs.apply(lambda x: x.datetime.year == 2024 and x.layer == 3, axis=1), :]
    ...
    if phi_file is not None:
        df = pd.read_csv(os.path.join(tpl_dir, phi_file), header=None, names=["tag", "prop"])
