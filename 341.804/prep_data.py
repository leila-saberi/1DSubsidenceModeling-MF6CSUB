...
def prep_data(use_delay, **kwargs):
    ...
    for k, aq in zip(uk, uaq):
        ...
        aobs_interp_smooth = aobs_interp.rolling(
            window=365, min_periods=1, center=True
        ).mean()
        aobs_interp.loc[aobs_interp.index.map(lambda x: x < smooth_thres), :] = (
            aobs_interp_smooth.loc[
                aobs_interp_smooth.index.map(lambda x: x < smooth_thres), :
            ]
        )
