...
def prep_data(use_delay, **kwargs):
    ...
    par_data = par_data.loc[:, ["parameter", "Lower", "Pliocene", "Santa Margarita"]]
    ...
    th_aq_dict = {"lower aquifer": 1, "pliocene": 2, "santa margarita": 3}

    interpolated_obs_dfs = []
    sm_df = obs.loc[obs.Aquifer == "Pliocene", :].copy()
    sm_df["Aquifer"] = "Santa Margarita"
    obs = pd.concat([obs, sm_df])
...
def modify_pst(tpl_dir):
    ...
    obs.loc[cobs2.index, "obsval"] = 10
    obs.loc[cobs2.index, "obgnme"] = "greater_than-compact2"
    obs.loc[cobs2.index, "weight"] = 0.0
    obs.loc[cobs3.index, "obsval"] = 60
