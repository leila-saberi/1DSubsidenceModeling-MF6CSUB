...
def modify_pst(tpl_dir):
    ...
    import pyemu
    location = "P692"
    pst = pyemu.Pst(os.path.join(location, tpl_dir, "pest.pst"))
    ...
    if phi_file is not None:
        df = pd.read_csv(
            os.path.join(location, tpl_dir, phi_file), header=None, names=["tag", "prop"]
        )

        if "less_than_rebound" not in df.tag.values:
            df.index = df.pop("tag")
            df.loc["prefer-compact3", "prop"] = 1
            df.to_csv(os.path.join(location, tpl_dir, phi_file), header=False)

    pst.control_data.noptmax = -2
    pst.write(os.path.join(location, tpl_dir, "pest.pst"), version=2)
